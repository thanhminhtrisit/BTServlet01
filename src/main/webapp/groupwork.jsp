			...
			<div class="container-fluid">
				<div class="row bg-title">
					...
					<div class="col-lg-9 col-sm-8 col-md-8 col-xs-12 text-right">
						<a href="groupwork-add.jsp" class="btn btn-sm btn-success">Thêm
							mới</a>
					</div>
					<!-- /.col-lg-12 -->
				</div>
						...
						<div class="white-box">
							<div class="table-responsive">
								<table class="table" id="example">
									...
									<tbody>
										<c:forEach var="job" items="${listJobs}" varStatus="status">
											<tr>
												<td>${status.index + 1}</td>
												<td>${job.name}</td>
												<td><fmt:formatDate value="${job.datestart_date}"
														pattern="yyyy-MM-dd" /></td>
												<td><fmt:formatDate value="${job.end_date}"
														pattern="yyyy-MM-dd" /></td>
												<td><a href="job-update?id=${job.id}"
													class="btn btn-sm btn-primary">Sửa</a> <a
													href="job-delete?id=${job.id}"
													class="btn btn-sm btn-danger"
													onclick="return confirm('Bạn có chắc muốn xóa job ${job.name} này không?');">Xóa</a>
													<a href="groupwork-details.html"
