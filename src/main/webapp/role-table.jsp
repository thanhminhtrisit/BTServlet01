						...
						<div class="white-box">
							<div class="table-responsive">
								<table class="table" id="example">
									...
									<tbody>
										<c:forEach var="role" items="${listRoles}" varStatus="count">
											<tr>
												<td>${count.index +1}</td>
												<td>${role.name}</td>
												<td>${role.description}</td>
												<td><a href="role-update?id=${role.id}" class="btn btn-sm btn-primary">Sửa</a>
													<a href="role-delete?id=${role.id}"
													class="btn btn-sm btn-danger"
													onclick="return confirm('Bạn có chắc muốn xóa role: ${role.name}?');">Xóa</a>
											</tr>
										</c:forEach>
									</tbody>
								</table>
